...
class TestResolveInputs:
 ...
 temp_dir):
    ...
    def test_folder_recursion(self,
 temp_dir):
    ...
    def test_folder_skips_excluded_dirs(self,
 temp_dir):
        result = resolve_inputs([temp_dir])
        names = {f.name for f in result}
        assert "junk.md" not in names

    def test_folder_skips_non_text_files(self,
 temp_dir):
        result = resolve_inputs([temp_dir])
        names = {f.name for f in result}
 ...
 temp_dir):
        ...
        result = resolve_inputs([file_path,
 folder_path])
        names = {f.name for f in result}
...
expected",
 [
        ("product-brief-foo.md",
 "product-brief"),

        ("product_brief_bar.md",
 "product-brief"),

        ("foo-discovery-notes.md",
 "discovery-notes"),

        ("foo-discovery_notes.md",
 "discovery-notes"),
...
class TestSuggestGroups:
    def test_groups_brief_with_discovery_notes(self,
 temp_dir):
        files = [
...
class TestAnalyze:
 ...
 output_file)
    ...
    def test_routing_single_small_input(self,
 temp_dir):
        f = str(Path(temp_dir) / "product-brief-foo.md")
        output_file = str(Path(temp_dir) / "output.json")
        analyze([f],
 output_file)
    ...
    def test_routing_fanout_many_files(self,
 temp_dir):
