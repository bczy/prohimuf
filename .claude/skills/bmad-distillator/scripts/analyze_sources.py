...
Enumerates files from paths/folders/globs,
 computes sizes and token estimates,

detects document types from naming conventions,
 and suggests groupings for
related documents (e.g.,
 a brief paired with its discovery notes).

Accepts: file paths,
 folder paths (scans recursively for .md/.txt/.yaml/.yml/.json),

or glob patterns. Skips node_modules,
 .git,
 __pycache__,
 .venv,
 _bmad-output.
...
Output JSON structure:
 ...
 total_estimated_tokens
  groups[]: group_key,
 files[] with role (primary/companion/standalone)
    - Groups related docs by naming convention (e.g.,
 brief + discovery-notes)
  routing: recommendation ("single" | "fan-out"),
 reason
    - single: ≤3 files AND ≤15K estimated tokens
    - fan-out: >3 files OR >15K estimated tokens
  split_prediction: prediction ("likely" | "unlikely"),
 reason,
...
GROUP_PATTERNS = [
    ...
    (r"^(.+?)(?:-discovery-notes|-discovery_notes)\.(\w+)$",
 r"\1.\2"),

    # base document + appendix
    (r"^(.+?)(?:-appendix|-addendum)(?:-\w+)?\.(\w+)$",
 r"\1.\2"),
...
def suggest_groups(files: list[Path]) -> list[dict]:
 ...
 list[dict]] = {}
    ...
    for f in files:
        ...
        matched = False
        for pattern,
 base_pattern in GROUP_PATTERNS:
            m = re.match(pattern,
 f.name,
 re.IGNORECASE)
            if m:
                # This file is a companion — find its base
                base_name = re.sub(pattern,
 base_pattern,
 f.name,
 flags=re.IGNORECASE)
                group_key = base_name
...
def analyze(inputs: list[str],
 ...
 ~{total_tokens:,
    ...
    if estimated_distillate_tokens > SINGLE_DISTILLATE_MAX_TOKENS:
        ...
        split_reason = (
            f"Estimated distillate ~{estimated_distillate_tokens:,
} tokens "
            f"exceeds {SINGLE_DISTILLATE_MAX_TOKENS:,
} threshold"
        )
    else:
        ...
        split_reason = (
            ...
            f"within {SINGLE_DISTILLATE_MAX_TOKENS:,
} threshold"
        )
