...
Analyze product brief,
 research,
 and brainstorming documents for user personas already defined.
...
- Suggest additional user types based on product context (admins,
 moderators,
 support,
 API consumers,
 internal ops)
...
- Consider beyond primary users: admins,
 moderators,
 support staff,
 API consumers,
 internal ops
...
For each user type,
 create compelling narrative journeys that tell their story:
...
- Craft journey narrative: where do we meet them,
 how does product help them write their next chapter?
...
Encourage narrative format with specific user details,
 emotional journey,
 and clear before/after contrast

### 3. Guide Journey Exploration

For each journey,
 facilitate detailed exploration:
...
- Connect journey needs to concrete capabilities (onboarding,
 dashboards,
 notifications,
 etc.)
...
- **Primary user** - edge case (different goal,
 error recovery)
...
Present the user journey content for review,
 then display menu:
...
- IF A: Invoke the `bmad-advanced-elicitation` skill with the current journey content,
...
- IF P: Invoke the `bmad-party-mode` skill with the current journeys,
...
- IF C: Append the final content to {outputFile},
 ...
 then read fully and follow: ./step-05-domain.md
...
❌ Not having enough journey diversity (admin,
 support,
 API,
 etc.)
...
3. **Admin/Operations User**: Management,
 configuration,
 monitoring
4. **Support/Troubleshooting**: Help,
 investigation,
 issue resolution
