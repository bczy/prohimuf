...
Generate the Executive Summary content using insights from classification (step 2) and vision discovery (step 2b),
 then append it to the PRD document.
...
- 🎯 Content must be dense,
 precise,
 and zero-fluff (PRD quality standards)
...
- Current document and frontmatter from steps 1,
 2,
 and 2b are available
- Project classification exists from step 2 (project type,
 domain,
 complexity,
 context)
- Vision and differentiator insights exist from step 2b
- Input documents from step 1 are available (product briefs,
 research,
 brainstorming,
 project docs)
...
Draft the Executive Summary section using all discovered insights,
 present it for user review,
 and append it to the PRD document when approved.
...
- Classification from step 2: project type,
 domain,
 complexity,
 project context
- Vision and differentiator from step 2b: what makes this special,
 core insight
- Input documents: product briefs,
 research,
 brainstorming,
 project docs
...
- Precise and actionable — clear,
 specific statements
- Dual-audience optimized — readable by humans,
 consumable by LLMs
...
Present the executive summary content for user review,
 then display menu:
...
- IF A: Invoke the `bmad-advanced-elicitation` skill with the current executive summary content,
...
- IF P: Invoke the `bmad-party-mode` skill with the current executive summary content,
...
- IF C: Append the final content to {outputFile},
 ...
 then read fully and follow: ./step-03-success.md
...
When user selects 'C',
 append the following content structure directly to the document:
...
- `{vision_alignment_content}` — Product vision,
 target users,
 and the problem being solved. Dense,
 precise summary drawn from step 2b vision discovery.
- `{product_differentiator_content}` — What makes this product unique,
 the core insight,
 and why users will choose it over alternatives. Drawn from step 2b differentiator discovery.
- `{project_classification_content}` — Project type,
 domain,
 complexity level,
...
ONLY WHEN [C continue option] is selected and [content appended to document],
 will you then read fully and follow: `./step-03-success.md` to define success criteria.
...
- Content meets PRD quality standards (dense,
 precise,
 zero-fluff)
...
**Master Rule:** Generate high-quality Executive Summary content from discovered insights. Present for review,
 refine collaboratively,
 and only save when the user approves. This is the first substantive content in the PRD — it sets the quality bar for everything that follows.
