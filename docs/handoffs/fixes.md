...
- 2026-07-30 · claude/spike-r3f-flyers · dev-tooling-assets · rangement de l'exploration R3F des flyers (mur + livre) hors du chemin de prod, sur demande de Bertrand qui ne voulait pas se prononcer : les 2 prototypes n'existaient QUE comme fichiers non suivis dans un worktree (~85 Ko, invisibles pour git) et disparaissaient à la suppression du worktree. Commités sous `spikes/r3f-flyers/` avec un README qui consigne ce que le spike démontre, les TROIS coûts qui l'ont laissé de côté (frontière menus-en-DOM, accessibilité d'un canvas face aux flyers DOM, chargement paresseux de Three.js d'ADR-0068), une piste intermédiaire non explorée (feuilles froissées pré-calculées en images) et les 5 pièges rencontrés (UV de `ExtrudeGeometry` en unités-monde ; verso à NE PAS construire par rotation de 180° sinon un trou en `+x` passe en `−x` et les faces ne coïncident jamais — utiliser `BackSide` + texture pré-inversée ; relief du froissé à garder sous l'écart entre feuilles empilées ; `state.clock.getDelta()` dans `useFrame` qui corrompt l'horloge des autres composants). Aucun fichier de `src/` touché, rien ne l'importe. Deux régressions que le premier commit avait introduites sur la branche, corrigées ensuite : `yarn lint` cassé repo-wide (fichiers hors du projet tsconfig ⇒ les règles typées ne les parsent pas) via `spikes/**` dans les ignores eslint, et `yarn format:check` cassé (sources non formatées) via prettier ; plus les 2 pages déclarées comme entrées de build Vite, sinon elles n'atteignaient jamais `dist` et leurs URLs de preview renvoyaient 404 — un spike rangé que personne ne peut ouvrir n'est pas archivé. Les deux changements de config portent la mention MUST NOT MERGE TO MAIN dans le fichier même · checks: eslint/prettier verts repo-wide + vérification contre le BUILD de prod (`vite preview` + Playwright SwiftShader) : les 2 pages chargent sans erreur console, et présence confirmée sur `gh-pages` puis HTTP 200 sur les 2 URLs publiques (le 404 initial était le cache Pages) · review: aucune — branche d'archive, pas de PR, aucune intention de merge
