...
## stage-6. FINAL RE-TRIAGE (fix diff) — senior-architect (Winston) — 2026-07-20

- claim: re-triage of the fix range `git diff 551c287..d6b31d1` (commits 268eb16 dev-gameplay,
  b9582ef dev-r3f-render, 2068588 tech-writer) against my NO-MERGE pre-merge set; verify the guards
  match the prescription and nothing else moved; confirm the re-verify sequence ran. / release:
  **MERGE.** All four pre-merge items landed exactly as prescribed, footprint is guards + tests +
  docs only, gates re-run green, QA re-verify PASS.
- VERDICT: MERGE — stage-6 panel (senior-architect)

**Fix diff verified against prescription (source-read, not on faith):**

- **MAJEUR (a)** — `lootSystem.attemptSpawn`: `occupied = new Set(enemies.filter(e => e.state !==
"DEAD").map(e => e.slotIndex))`, applied **alongside** (`!occupied.has(slotIndex) &&
canSpawnLootAt(...)`) the §5.4 column-gap rule. Catches the HIDDEN/HIT cases the column rule
  missed. Exactly as prescribed.
- **MAJEUR (b)** — `enemySystem.spawnWave`: optional `excludeSlots` param, excluded slots dropped
  **before** the `count` slice; empty ⇒ legacy path byte-identical (shuffle + per-position seed
  untouched). `stateMachine` rollover passes `state.loot !== null ? [state.loot.slotIndex] : []`; the
  construction call correctly passes nothing (loot is null at init). **No `bulletSystem` change** —
  the tie-break assumption is restored true by the upstream guards, as designed.
- **MINEUR-1** — `useGameLoop`: raw-gesture `playSfx("shoot")` removed; cue fires post-tick off
  `next.impactEvents.length > 0`. Base/miss → 1 (unchanged), burst → one pop per round-tick, spread →
  1/press, swallowed/refractory tap → silent. Base/no-loot byte-identical.
- **MINEUR-2** — `stateMachine.createInitialState`: `_nextLootId = 1` reset; the "deterministic,
  replay-safe" claim is now true (fresh sessions reproduce the crate sequence).
...
**Nothing else moved:** the range is the five files above + tests
(`{lootSystem,stateMachine,enemySystem}.test.ts`, +7 → 867) + docs (ADR, fixes.md, this shard, qa
plan, adr index/html). No new production surface, no deps, no deploy change.

**Gates + QA re-verify (accepted, read not re-run by me):** tsc/lint/format clean · 867/867 · goldens
PASS (stalingrad 7.392 %, vitry 6.661 % — the universal HUD weapon cell, the expected shipped-level
delta) · qa-lead VERDICT PASS (624 crate-state reads, 0 co-location violations; live pickup clean).
The two MANUAL items are **not gate holes** and I concur: the wave-rollover-with-crate coincidence is
covered by the new `stateMachine`/`enemySystem` unit tests (direction b), and a 0.45 s one-shot fade
is not reliably capturable in the SwiftShader sandbox while the CSS rule itself is directly
inspectable (MINEUR-3 fix verified by source, not screenshot). No CONFIRMED pre-merge finding remains
open. CI green on head d6b31d1.
