...
| `src/render/scene/NearForeground.tsx`                           | **CORRIGÉ (panel PR #143, BLOQUANT) — la description précédente était fausse.** Elle annonçait « erreur ESLint pré-existante — 1 ligne ». Le diff réel contre `origin/main` fait ~30 lignes et change le RENDU : nouvelle prop `stateRef?: RefObject<GameState>` (doc de 23 lignes, `:85-107`), câblage depuis `GameScene.tsx:452`, et comportement par frame (`:295-298`) masquant les rangées de props de trottoir dès qu'un boss QTE est actif. C'est le correctif de z-order d'une session concurrente (rapport Bertrand du 26/07 : le lampadaire peinturait par-dessus le Commandant figé) — visible par le joueur, et **non relu**. Le correctif ESLint 1 ligne existe, mais il ne représente qu'une fraction du hunk. **Tranché par Bertrand le 29/07 : assumé comme chantier de plein droit**, tracé dans [`story-boss-tableau-kerb-occlusion.md`](./story-boss-tableau-kerb-occlusion.md) — donc plus « hors story », mais une story à part entière. |
