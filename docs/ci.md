...
| **Cleanup branch preview**   | `cleanup-preview.yml`     | branch delete, PR close, manual    | Removes `preview/<branch>/` from `gh-pages` once its PR is closed (merged or not) or the branch is deleted                            |
...
- **Cleanup** — **Cleanup branch preview** (`cleanup-preview.yml`) removes
  `preview/<slug>/` as soon as its PR is closed (merged **or** closed unmerged)
  or the branch is deleted — a preview lives only while its PR is open. It
  publishes an _empty_ directory through the same `gh-pages-publish`
  clean-replace (git keeps no empty dirs, so the subtree disappears — same
  rebase-retry loop, no race with in-flight deploys). Previews orphaned before
  this workflow existed can be pruned manually (Actions → Cleanup branch
  preview → branch name).
...
| Remove a stale branch preview                | Actions → Cleanup branch preview → branch name (automatic on PR close / branch delete)            |
