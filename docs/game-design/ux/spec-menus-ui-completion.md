...
- [ ] **Visible text state.** Every ballot option already prints its label; the _selected_
...
A total first-time player has no signal that the manila flyer is different in _kind_
...
- **First-ever-session auto-focus (not auto-navigate).** While `muf_seen_tutorial_nudge` is
  ...
  new visual language. This is _focus_, not _navigation_: nothing plays, nothing advances, the
...
difficulty _stamp_. It belongs where the player is already making a "which gig tonight"
...
| Case                                                        | Behavior                                                                                                                                                                 |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Player changes PRESSION from the NIVEAUX header, then plays | Applies at the next `handlePlay` (`buildLevelParams` reads `prefs.difficulty` fresh at level start) — already true today, unaffected by _where_ the control lives.       |
