...
| Le coup fatal dépasse les cœurs restants (CRS à 1,0 sur 0,5 cœur restant)        | La contribution du dernier tick est **écrêtée au reste de la jauge**. Le total ne peut jamais dépasser les cœurs de départ : `3,25 / 3` se lirait comme un bug. Plafond = valeur de départ de la jauge (3 par défaut, 1 à 5 selon la préférence joueur). |
...
| AC-6  | Les cœurs perdus **n'excèdent jamais** la valeur de départ de la jauge, y compris sur un coup fatal surdimensionné.                                                            | Test déterministe (CRS à 1,0 sur 0,25 cœur restant ⇒ `3`, pas `3,75`). |
