...
peut donc pas _shipper_ le boss dans un niveau qui n'existe pas. Cette contrainte renforce mon
...
- **Ligne 6** enseigne la règle **diégétiquement** (« vulnérable quand il ouvre le feu ») dans
  ...
  choisira pour Open Q2 (elle nomme le _quand_, pas le _comment_).
...
6. **Ne décide PAS** Open Q1 (gate vs bonus — `game-designer`/gate), Q2 (forme de la fenêtre —
   ...
   fiction est **compatible avec les deux lectures d'Open Q1** ; elle _penche_ pour le beat requis
...
- **Boucle intouchable** — le boss branche sur `Récupérer → Livrer` (le _comment_ est Open Q1,
...
avec `game-designer` (Sacha) : la fenêtre de vulnérabilité (Q2) doit _pouvoir_ se lire comme
