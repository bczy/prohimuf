...
2. **Its consequence lands on a level that does not exist yet** (A1c). `energy` is a field of
   ...
   _inoperative_ (clamped at 100) — which is exactly why A1c deletes it and keeps only the
   `FAILED` malus.
...
button gone, the scene resolves _by itself_ the instant the four bands are right. The victory
...
all defaulting to a byte-identical pre-feature build. And a run today is _one attempt on one
level_ (ADR-0076 F1): "the next level" is, concretely, the next `PLAYING` boot in the same
session.
...
2. **Accessibility is a hard requirement here** (gate A7 `aria-live` chrono announcements, UX
   44×44 px targets, the chevrons that A4-bis demotes to _accessibility targets_). DOM gives
...
post-composition, one layer, zero per-band cost. Rendering the scene inside the Canvas _purely_
...
`PortraitScene` is a **standalone immutable record**, _not_ a field of `GameState`. It is
...
### D4 — The verdict travels as a `LevelModifier`, applied at the _next_ `createInitialState`
...
1. **The rule** is `levelModifierFromPortrait(result): LevelModifier`, pure, in
   `src/game/systems/portraitRobotSystem.ts`. It is the _only_ place the gate's §3 payoff table
...
3. **The application point** is `createInitialState`, through **one new optional field on
   ...
   - `energy: applyEnergy(ENERGY_INITIAL, modifier?.energyDelta ?? 0)` — the existing clamp is
     ...
     hypothetical future bonus is silently clamped (which is _why_ A1c deleted the reward).
...
keeps running is _less pressure and less kill time_ on a quota level. That is the payoff working
...
id today. The render lane picks _a scene by key_; it does not author or branch on the verdict's
meaning.
...
- `usePortraitRobot` takes a `paused: boolean`; `App.tsx` passes `rotateBlocked` (its existing
  ...
  inbox is cleared (D8.3), so the chrono is frozen by _not advancing_, never by a special case
...
que si aucun verrouillage n'a eu lieu" is not a check written somewhere, it is the _domain_ of
...
non-determinism, and it lives in the _sequencing_, so the sequencing must leave the hook:
...
- **The hook has nothing left to order**, which is the real protection: an ordering discipline
  a hook is _asked_ to respect is a comment; one it _cannot express_ is an architecture.
...
strictly stronger guarantee: a time-based guard bounded _when_ an accidental validation could
...
_change_, never to a comparison of their own. Three properties fall out for free: the crossing
...
is deliberate; it is made safe by the modifier being an _opaque, closed_ value type the shell
...
is safe _only_ because of D8's three guarantees together — resolution is a reducer
...
Building the timeline _now_, for one consumer, is speculative generality — flagged here so the
...
_rule of the game_ (which outcome wins at equality) in `src/hooks`, where it is untested,
invisible to `src/game`'s test suite, and one refactor away from being reordered by someone
tidying an effect. Worse, it is the kind of correctness that _looks_ like plumbing — nobody
...
differently on two machines. Determinism by _fold order_ needs no clock at all.
...
_button_ against an accidental press; there is no button. Applied to auto-resolution it would
