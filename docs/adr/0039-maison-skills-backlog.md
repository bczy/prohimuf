...
| `simplify`     | owning dev lane      | End of stage 5, before the panel                        | Degrease the branch diff before review: hunt the weight the change added (speculative abstraction, single-use indirection, impossible-state guards, duplicated constants), apply only the behaviour-preserving cuts proven green, hand the judgement calls back to the lane. | High     | ✅ shipped          |
| `open-pr`      | any lane             | After a push to `claude/**`                             | Compute the preview slug, fill `.github/pull_request_template.md`, open the **draft** PR, subscribe to activity.                                                                                                                                                             | Medium   | ✅ shipped          |
| `fix-lane`     | dev lane             | Small single-lane already-gated diff                    | Run the two-tier fix path: tsc/vitest/lint → one `code-review` (high) → one line in `docs/handoffs/fixes.md`; escalate to the full pipeline the moment a criterion breaks.                                                                                                   | Medium   | ✅ shipped          |
| `root-cause`   | owning dev lane      | Any defect, before writing the fix                      | Reproduce as a failing test → trace to the ONE mechanism → name the cause and why no test caught it → fix the cause, keep the reproduction as the regression test. Kills the symptomatic patch.                                                                              | High     | ✅ shipped          |
| `test-quality` | `qa-lead`            | Stage 5, on any diff that adds/changes tests            | Break the source on purpose (throwaway, reverted probes) and check the new tests go red for the right reason: BITES / SURVIVES / NOISY, feeding the QUALITY GATE. Coverage stays a secondary signal.                                                                         | High     | ✅ shipped          |
