...
// render feedback (full-screen red flash, camera shake — ADR-0064 D2). Mirror
// of ImpactEvent in the opposite direction (enemy → player). Cosmetic-only:
// the rule change on `lives` still lives in stateMachine. Consumed per-frame
// by the hooks bridge; never persisted.
export interface PlayerHitEvent {
  // World point at which the enemy bullet crossed the player-hit disc. The
  // player is conceptually at the origin (see PLAYER_HIT_RADIUS around
  // (0, 0)), so |worldPoint| ≤ PLAYER_HIT_RADIUS.
