...
@keyframes gi-tt-lift { 0%{transform:translateY(0)} 28%{transform:translateY(0)} 50%{transform:translateY(-12px)} 72%{transform:translateY(-12px)} 100%{transform:translateY(0)} }
@keyframes gi-tt-halo { 0%{opacity:.2} 8%{opacity:1} 22%{opacity:1} 45%{opacity:.2} 100%{opacity:.2} }
@keyframes gi-tt-ripple { 0%{opacity:0;transform:scale(.3)} 6%{opacity:.85} 28%{opacity:0;transform:scale(1.25)} 100%{opacity:0;transform:scale(1.25)} }
...
function TwoFingerTapIcon(): JSX.Element {
  return (
    <svg viewBox="0 0 120 120" style={ICON_SVG_STYLE} aria-hidden="true">
      ...
      {/* glass: inert */}
      <rect x="16" y="12" width="88" height="96" rx="10" fill={BODY} stroke={INK} strokeWidth="2" opacity="0.9" />
      {/* single ripple from the midpoint between the two tips */}
      <circle
        ...
        opacity="0"
      />
      {/* both fingers descend/lift together; both fingertip halos share ONE class → flash in sync */}
      <g className="gi-anim gi-tt-lift">
        <rect x="41" y="58" width="15" height="46" rx="7.5" fill={BODY} stroke={INK} strokeWidth="2" transform="rotate(-9 48 82)" />
        <rect x="64" y="58" width="15" height="46" rx="7.5" fill={BODY} stroke={INK} strokeWidth="2" transform="rotate(9 72 82)" />
        <circle className="gi-anim gi-tt-halo" cx="49" cy="60" r="13" fill="url(#gi-tt-halo-grad)" opacity="0.5" />
        <circle className="gi-anim gi-tt-halo" cx="71" cy="60" r="13" fill="url(#gi-tt-halo-grad)" opacity="0.5" />
      </g>
    </svg>
  );
}
...
function SwipePanIcon(): JSX.Element {
  return (
    <svg viewBox="0 0 120 120" style={ICON_SVG_STYLE} aria-hidden="true">
      ...
      {/* glass: inert */}
      <rect x="14" y="14" width="92" height="92" rx="10" fill={BODY} stroke={INK} strokeWidth="2" opacity="0.6" />
      {/* horizontal sweep: fingertip + motion trail (bright at tip → alpha 0 at tail) */}
      <g className="gi-anim gi-sp-h">
        <rect className="gi-anim gi-sp-h-trail" x="18" y="55" width="34" height="10" rx="5" fill="url(#gi-sp-trail-h)" />
        <circle className="gi-anim gi-sp-h-tip" cx="52" cy="60" r="9" fill="url(#gi-sp-tip-grad)" />
      </g>
      {/* vertical sweep (second half of the cycle → conveys pan in other directions) */}
      <g className="gi-anim gi-sp-v" opacity="0">
        <rect className="gi-anim gi-sp-v-trail" x="55" y="18" width="10" height="34" rx="5" fill="url(#gi-sp-trail-v)" />
        <circle className="gi-anim gi-sp-v-tip" cx="60" cy="52" r="9" fill="url(#gi-sp-tip-grad)" />
      </g>
    </svg>
  );
}
