...
import type { Group, Mesh, MeshBasicMaterial, OrthographicCamera, Texture } from "three";
 ...
 *   - the headlight lift (`headlightsLit`, the only visual allowed to encode the sound
 *     cover — D-J / R3-2 / N-1) is a material COLOUR modulation: zero extra draw call and
 *     zero added blended coverage, strictly cheaper than a scoped additive quad;
...
/** Plate tint at rest and under the packet's headlights — a LIFT, not a colour code. */
const PLATE_TINT_DARK = 0.72;
const PLATE_TINT_LIT = 1;
...
export function PhotoQteView({
  ...
  useFrame(() => {
    ...
    const lift = current.headlightsLit ? PLATE_TINT_LIT : PLATE_TINT_DARK;
    material.color.setScalar(lift);
  });
  ...
  return (
    <group ref={groupRef} renderOrder={1000}>
      ...
      <mesh ref={plateRef} renderOrder={1000}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial
          map={plateTexture}
          color={PLATE_TINT_DARK}
          depthTest={false}
          depthWrite={false}
          toneMapped={false}
        />
