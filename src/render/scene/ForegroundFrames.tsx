...
/**
 * Build a transparent overlay texture: for each window zone, an ornate
 * wrought-iron balcony railing (rails + balusters + spear finials + a row of
 * decorative scrolls). Drawn in facade-image space (y-down) so it lines up with
 * the zones the cops sit in. The actual drawing lives in {@link drawForegroundIronwork}
 * so it can be reused by offline preview scripts.
 */
function buildFrameTexture(zones: readonly WindowZone[]): CanvasTexture | null {
  ...
  drawForegroundIronwork(g, zones, TEX_W, TEX_H);
