...
/**
 * Dessin procédural des façades — code Canvas2D pur, sans React ni Three.
 * Partagé entre le composant `TiledFacade` (navigateur) et les scripts de
 * rendu hors-ligne (Node + node-canvas), qui injectent leur propre `factory`.
 */
...
export function makeNormalCanvas(
  ...
  const normal = factory(W, H);
  const nctx = normal.getContext("2d");
  if (nctx === null) return normal;
...
export function makeFacadeCanvas(
  map: TileMap,
  factory: CanvasFactory = browserCanvas,
): HTMLCanvasElement {
