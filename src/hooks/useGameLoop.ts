...
/**
 * Bridge→render transport for enemy-bullet hits on the player (ADR-0064 D2).
 * Mirror of `ImpactChannel` in the opposite direction (enemy → player). Same
 * contract: per-frame queue + reset signal bumped on level restart.
 */
...
export function useGameLoop(
  ...
  useFrame((_state, delta) => {
    ...
    // Enemy → player hits: same drain pattern, mirror direction (ADR-0064 D2).
