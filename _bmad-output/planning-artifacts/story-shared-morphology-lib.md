...
- **AC1 (one module):** `scripts/lib/morphology.mjs` holds the single implementation (disk offsets, dilate, erode, border-seeded hole fill (`fillHoles`), connected-component labeling, largest-component, solid-body-mask). All duplicated/hand-rolled copies deleted; every consumer (`fill-sprite-holes`, `retouch-flash-halos`, `measure-muzzle-anchors`, `check-sprite-integrity`, `restore-figure-bites`, `fill-bust-hem`) imports it. `cutout-enemies` is EXEMPT by architect decision (its floods are fused with colour sampling and map to no mask primitive; stays local with a pointer comment). `computeDeletions`' candidate-aware exterior flood in `retouch-flash-halos` is likewise local by design — its passability (transparent OR candidate) duplicates no lib function.
...
- **AC3 (connectivity explicit):** `labelComponents` carries the explicit `4|8` connectivity option; `fillHoles` and `largestComponent` are intentionally 4-conn-only (explicit via docstring + call-site comments, not a speculative parameter). Each call site carries a comment stating which connectivity it relies on and why (must match today's de-facto behaviour exactly; this is documentation, not a tuning change).
