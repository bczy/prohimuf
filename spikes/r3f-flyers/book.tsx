...
function makeCardTexture(f: FlyerSpec, i: number): THREE.CanvasTexture {
  ...
  // Wear along the pocket folds: the crease rubs darker and the ink lifts right on it.
  // Drawn LAST so it ages the print too, not just the stock — a fold that spared the
  ...
  foldGrime(c.width / 2, 0, c.width / 2, c.height, 16);
  foldGrime(0, c.height / 2, c.width, c.height / 2, 14);
 ...
 * whatever the holder scribbled on it during the night. Same one-ink constraint as
 * the fronts — a two-sided colour job was out of the question at this price.
 * ═══════════════════════════════════════════════════════════════════════════════ */
...
function drawFigure(
  ...
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.arc(x, y - s * 0.75, s * 0.24, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.moveTo(x, y - s * 0.5);
  ctx.lineTo(x, y + s * 0.25);
  const armL = seeded(seed, 1) * 0.9 - 0.45;
  const armR = seeded(seed, 2) * 0.9 - 0.45;
  ctx.moveTo(x - s * 0.45, y - s * 0.1 + armL * s);
  ctx.lineTo(x, y - s * 0.25);
  ctx.lineTo(x + s * 0.45, y - s * 0.1 + armR * s);
...
function drawBalloon(
  ...
  for (let r = 0; r < rows; r++) {
    const ly = y - h * 0.18 + (r * h) / 5;
    const lw = w * (0.3 + seeded(seed, r + 5) * 0.34);
    ctx.beginPath();
...
/** Speed lines / impact rays — the cheapest way to imply motion in one ink. */
function drawRays(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, n: number): void {
  ctx.save();
  ctx.strokeStyle = INK;
  ctx.lineWidth = 2.5;
  for (let k = 0; k < n; k++) {
...
/** A page of fake bande dessinée: panel grid, figures, balloons, onomatopoeia. */
function drawFakeBD(ctx: CanvasRenderingContext2D, cw: number, ch: number, seed: number): void {
  const m = 40;
  const cols = 2;
  const rows = 3;
  const gap = 16;
  const pw = (cw - m * 2 - gap * (cols - 1)) / cols;
  const ph = (ch - m * 2 - gap * (rows - 1)) / rows;
  const SFX = ["BOUM", "TCHAK", "VRRR", "CLAC", "BADABOUM", "PSCHIT"];

  for (let r = 0; r < rows; r++) {
    for (let cidx = 0; cidx < cols; cidx++) {
      const px = m + cidx * (pw + gap);
      const py = m + r * (ph + gap);
      const k = r * cols + cidx;
      ...
      // Clip so nothing leaks between panels — a real repro would have.
      ctx.beginPath();
      ctx.rect(px, py, pw, ph);
      ctx.clip();

      const pick = seeded(seed, 40 + k);
      if (pick < 0.3) {
        // Wide shot: horizon line + two figures.
        ctx.strokeStyle = INK;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ...
        ctx.stroke();
        drawFigure(ctx, px + pw * 0.32, py + ph * 0.6, ph * 0.2, seed + k);
        drawFigure(ctx, px + pw * 0.68, py + ph * 0.62, ph * 0.18, seed + k + 7);
      } else if (pick < 0.55) {
        // Close-up with a balloon.
        drawFigure(ctx, px + pw * 0.3, py + ph * 0.66, ph * 0.28, seed + k);
        drawBalloon(ctx, px + pw * 0.62, py + ph * 0.3, pw * 0.6, ph * 0.34, seed + k);
      } else if (pick < 0.78) {
        // Impact panel: rays + onomatopoeia.
        drawRays(ctx, px + pw / 2, py + ph / 2, Math.min(pw, ph) * 0.46, 14);
        ctx.fillStyle = INK;
        const word = SFX[Math.floor(seeded(seed, 60 + k) * SFX.length)];
        let fs = 54;
        ctx.font = `bold ${fs}px monospace`;
        while (ctx.measureText(word).width > pw * 0.8 && fs > 18) {
          fs -= 3;
          ctx.font = `bold ${fs}px monospace`;
        }
        ctx.save();
        ctx.translate(px + pw / 2, py + ph / 2);
        ctx.rotate((seeded(seed, 70 + k) - 0.5) * 0.5);
        ctx.fillText(word, -ctx.measureText(word).width / 2, fs * 0.35);
        ctx.restore();
      } else {
        // Silent panel: a halftone mass, someone's back turned.
        drawHalftoneBlock(ctx, px, py, pw, ph, seed + k);
        drawFigure(ctx, px + pw * 0.5, py + ph * 0.7, ph * 0.26, seed + k);
      }
      ctx.restore();
    }
  }
...
function drawClassifieds(ctx: CanvasRenderingContext2D, cw: number, ch: number, seed: number): void {
  const ADS = [
    ...
    "DONNE VINYLES HARDCORE · 06 · · · ·",
    "RÉPARE AMPLIS · RAPIDE · PAS CHER",
    "CHERCHE LIEU · GRAND · DISCRET",
    "VDS GÉNÉRATEUR 6KW · ROULE",
    "COVOIT. PORTE DE LA CHAPELLE · 23H",
    ...
    "CHERCHE PLATINE DE REMPLACEMENT",
    "TATOUEUR · À DOMICILE · SÉRIEUX",
    "VDS DISQUE DUR SAMPLER · 1200 F",
    "CHERCHE COVOIT. VERS LE 94 · SAM.",
    "IMPRIME TES AFFICHES · GROS TIRAGE",
    "RECHERCHE CHIEN PERDU · RÉPOND À MUF",
  ];
  // Sample WITHOUT replacement (seeded Fisher–Yates): drawing independently per line
  // repeated the same ad three or four times down one column, which no real classifieds
  // page ever does and which the eye catches immediately.
  const order = ADS.map((_, n) => n);
  for (let n = order.length - 1; n > 0; n--) {
    const j = Math.floor(seeded(seed, 200 + n) * (n + 1));
    [order[n], order[j]] = [order[j], order[n]];
  }
  ctx.save();
  ctx.fillStyle = INK;
  ctx.font = "bold 30px monospace";
  ctx.fillText("PETITES ANNONCES", 40, 68);
  ctx.strokeStyle = INK;
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(40, 84);
  ...
  let y = 130;
  for (let k = 0; k < order.length && y < ch - 60; k++) {
    ctx.fillText(ADS[order[k]], 46, y);
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(40, y + 14);
    ctx.lineTo(cw - 40, y + 14);
    ctx.stroke();
    y += 46;
  }
  ctx.restore();
...
/** A photocopied street map, hand-marked with the RV cross. */
function drawMap(ctx: CanvasRenderingContext2D, cw: number, ch: number, seed: number): void {
  ctx.save();
  ctx.strokeStyle = INK;
  ctx.lineWidth = 2;
  for (let k = 0; k < 9; k++) {
    const x = 40 + seeded(seed, 90 + k) * (cw - 80);
    ctx.beginPath();
    ctx.moveTo(x, 40);
    ctx.lineTo(x + (seeded(seed, 100 + k) - 0.5) * 160, ch - 40);
    ctx.stroke();
  }
  for (let k = 0; k < 7; k++) {
    const y = 60 + seeded(seed, 110 + k) * (ch - 120);
    ctx.beginPath();
    ctx.moveTo(40, y);
    ctx.lineTo(cw - 40, y + (seeded(seed, 120 + k) - 0.5) * 120);
    ctx.stroke();
  }
  // The hand-drawn cross + note, in a heavier "felt-tip" weight.
  const mx = cw * (0.35 + seeded(seed, 130) * 0.3);
  const my = ch * (0.4 + seeded(seed, 131) * 0.25);
  ctx.lineWidth = 9;
  ctx.beginPath();
  ctx.moveTo(mx - 46, my - 46);
  ctx.lineTo(mx + 46, my + 46);
  ctx.moveTo(mx + 46, my - 46);
  ctx.lineTo(mx - 46, my + 46);
  ctx.stroke();
  ctx.fillStyle = INK;
  ctx.font = "italic bold 30px monospace";
  ctx.fillText("ICI !!", mx + 58, my + 10);
  ctx.font = "20px monospace";
  ctx.fillText("suivre le canal", 50, ch - 60);
  ctx.restore();
}

/** Scrawled notes: phone numbers, a set-time list, doodles. */
function drawScrawl(ctx: CanvasRenderingContext2D, cw: number, ch: number, seed: number): void {
  const NOTES = [
    "01 42 · · · · rappeler",
    "KENZA → 23h porte est",
    "prendre la petite clé",
    "ne PAS passer par le quai",
    "2 caisses + la sono",
    "essence : 180 F",
    "dire à D. que c'est bon",
  ];
  ctx.save();
  ctx.fillStyle = INK;
  for (let k = 0; k < NOTES.length; k++) {
    const x = 50 + seeded(seed, 140 + k) * 90;
    const y = 110 + k * 108 + seeded(seed, 150 + k) * 24;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate((seeded(seed, 160 + k) - 0.5) * 0.16);
    ctx.font = `italic ${24 + Math.floor(seeded(seed, 170 + k) * 12)}px monospace`;
    ctx.fillText(NOTES[k], 0, 0);
    ctx.restore();
  }
  // A couple of absent-minded doodles.
  drawSpiral(ctx, cw * 0.74, ch * 0.24, 54);
  drawInvader(ctx, cw * 0.3, ch * 0.82, 46);
  ctx.restore();
}
...
function makeBackTexture(f: FlyerSpec, i: number): THREE.CanvasTexture {
  ...
  // Pre-mirror everything. The sheet's back faces are viewed from behind, which
  // reverses the texture exactly as looking at a poster through the paper would;
  // flipping the artwork here cancels that out so the lettering reads the right way.
  ctx.translate(c.width, 0);
  ctx.scale(-1, 1);

  // Same photocopy grime as the front, at lower contrast — the back got less ink.
  ctx.strokeStyle = "rgba(0,0,0,0.035)";
  ctx.lineWidth = 1;
  for (let y = -c.height; y < c.width + c.height; y += 9) {
    ctx.beginPath();
    ctx.moveTo(y, 0);
    ctx.lineTo(y + c.height, c.height);
    ctx.stroke();
  ...
  const kind = i % 4;
  if (kind === 0) drawFakeBD(ctx, c.width, c.height, i * 13 + 3);
  else if (kind === 1) drawClassifieds(ctx, c.width, c.height, i * 13 + 3);
  else if (kind === 2) drawMap(ctx, c.width, c.height, i * 13 + 3);
  else drawScrawl(ctx, c.width, c.height, i * 13 + 3);

  // Fold wear, as on the front.
...
/* ═══════════════════════════════════════════════════════════════════════════════
 * THE BOOK
 * Same sheets as the wall version, bound at a spine. A leaf swings around the spine,
 * so the geometry is offset by half a page and the GROUP rotates — rotating the mesh
 * about its own centre would pivot it through the middle of the page instead.
 * ═══════════════════════════════════════════════════════════════════════════════ */
...
/**
 * Depth between two stacked leaves. Must exceed the crumple relief in
 * `makeLeafGeometry`, or a crumpled page pushes through the one beneath it — which is
 * exactly what happened when the crumple was restored against a 0.022 gap.
 */
const LEAF_GAP = 0.16;

/**
 * Top face of the book's slab. Every leaf must rest ABOVE it: with five leaves per
 * stack the deepest sits at -4 × LEAF_GAP, and a leaf that dips below the slab is
 * swallowed by it — which is why turned pages were showing only as a sliver.
 */
const SLAB_TOP = -(4 * 0.16) - 0.25;

/**
 * A bound leaf is the SAME sheet as on the wall — torn edges, slit, crumple, dense
 * grid — not a fresh plane. Building a plain PlaneGeometry here (as an earlier pass
 * did) silently threw all of that away: the damage lives in the mesh, so a new mesh
 * has none of it. Reuse the wall builder, scale it to page size, then add the bow.
 */
function makeLeafGeometry(f: FlyerSpec, i: number): THREE.BufferGeometry {
  const geo = makeCrumpledGeometry(i, { tear: f.tear, slit: f.slit });
  geo.scale(PAGE_W / W, PAGE_H / H, 1);
  const pos = geo.attributes.position;
  for (let v = 0; v < pos.count; v++) {
    const x = pos.getX(v);
    // Pages bow away from the spine: the outer edge lifts, as a bound page does.
    const fromSpine = (x + PAGE_W / 2) / PAGE_W;
    const bow = Math.sin(fromSpine * Math.PI * 0.5) * 0.07;
    // Crumple kept, but scaled to the STACK's budget: a leaf whose relief is deeper
    // than the gap between leaves pokes through its neighbour. This factor and
    // LEAF_GAP below are one decision — change either and they must be re-matched.
    pos.setZ(v, pos.getZ(v) * 0.35 + bow);
  }
...
interface LeafProps {
  f: FlyerSpec;
  i: number;
  /** 0 = lying on the right stack, 1 = fully turned onto the left stack. */
  turn: number;
  /** Resting depth within its stack. Without it every leaf is coplanar at z=0 and the
   *  five pages z-fight into a patchwork instead of stacking. */
  restZ: number;
  bump: THREE.Texture;
  onClick: () => void;
}

function Leaf({ f, i, turn, restZ, bump, onClick }: LeafProps) {
  const pivot = useRef<THREE.Group>(null);
  const front = useMemo(() => makeCardTexture(f, i), [f, i]);
  const back = useMemo(() => makeBackTexture(f, i), [f, i]);
  const geometry = useMemo(() => makeLeafGeometry(f, i), [f, i]);
  const shown = useRef(turn);

  useFrame((_s, delta) => {
    if (!pivot.current) return;
    // Ease toward the target so a click reads as a page being pulled over, not a cut.
    shown.current += (turn - shown.current) * Math.min(1, delta * 4.5);
    const t = shown.current;
    pivot.current.rotation.y = -t * Math.PI;
    // Lift while in flight so the leaf clears both stacks, then settle onto its own
    // depth in the stack (restZ) rather than back onto the shared z=0 plane. The lift
    // is staggered PER LEAF: when several pages turn at once they would otherwise all
    // fly at the same height and cut through one another.
    const lift = Math.sin(t * Math.PI);
    pivot.current.position.z = restZ + lift * (1.1 + i * 0.22);
    pivot.current.rotation.z = lift * 0.05;
  });

  return (
    <group ref={pivot} onClick={onClick}>
      {/* Offset by half a page so the group's origin sits ON the spine. */}
      <group position={[PAGE_W / 2, 0, 0]}>
        ...
        {/* The back is the SAME geometry, in the SAME place, drawn via its BACK faces.
            Rotating a copy 180° (as an earlier pass did) sends a hole at +x to -x, so
            the two sides' damage could never line up. FrontSide and BackSide never
            rasterise the same pixel, so the two meshes cannot z-fight either. The
            artwork is pre-mirrored in makeBackTexture to undo the reversed view. */}
...
function BookScene({ turned, onTurn }: { turned: number; onTurn: (n: number) => void }) {
  ...
  useFrame((_s, delta) => {
    ...
    rig.current.rotation.y += (pointer.x * 0.13 - rig.current.rotation.y) * k;
    rig.current.rotation.x += (-0.13 - pointer.y * 0.07 - rig.current.rotation.x) * k;
  });

  return (
    <>
      <mesh position={[0, 0, SLAB_TOP - 1.4]} receiveShadow>
        <planeGeometry args={[60, 34]} />
        <meshStandardMaterial color="#efece2" roughness={1} />
      </mesh>
      ...
      />
      <pointLight position={[-7, -3, 4]} intensity={9} color="#ff4fa3" distance={13} decay={2} />
      <pointLight position={[7, -4, 4]} intensity={8} color="#b7f32b" distance={13} decay={2} />

      <group ref={rig}>
        {/* Slab under the leaves — the rest of the book's bulk. Its TOP face sits at
            SLAB_TOP, i.e. below the deepest leaf in either stack. */}
        <mesh position={[0, 0, SLAB_TOP - 0.2]} receiveShadow>
          <boxGeometry args={[PAGE_W * 2.04, PAGE_H * 1.03, 0.4]} />
          <meshStandardMaterial color="#e6e1d4" roughness={1} />
        </mesh>
        <mesh position={[0, 0, SLAB_TOP + 0.02]} receiveShadow>
          <boxGeometry args={[0.1, PAGE_H * 1.02, 0.1]} />
          <meshStandardMaterial color="#c9c3b4" roughness={1} />
        </mesh>

        {FLYERS.map((f, i) => {
          const isTurned = i < turned;
          // Depth within the stack. On the RIGHT the next page to turn must be on top,
          // so pages get deeper as the index grows; on the LEFT the most recently
          // turned page is on top, so the order reverses.
          const depth = isTurned ? turned - 1 - i : i - turned;
          return (
            <Leaf
              key={f.title}
              f={f}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? "transparent" : "#161514",
        color: disabled ? "rgba(22,21,20,0.28)" : "#d7d2c6",
        border: "2px solid",
        borderColor: disabled ? "rgba(22,21,20,0.28)" : "#161514",
      }}
    >
      {label}
    </button>
...
function BookPage() {
  const [turned, setTurned] = useState(2);

  // Arrow keys too — leafing through a book with ← → is the obvious reflex.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") setTurned((n) => Math.max(0, n - 1));
    ...
    <div style={{ fontFamily: "monospace", background: "#d7d2c6", minHeight: "100vh" }}>
      ...
      >
        ...
        <span style={{ fontSize: 11, opacity: 0.75, letterSpacing: "0.08em" }}>
          UNDERGROUND PARIS · FANZINE CLANDESTIN · 1998 — LE LIVRE : clique une page pour la tourner
        </span>
      </header>
      <div style={{ width: "100%", height: "74vh" }}>
        <Canvas
          shadows="soft"
          camera={{ fov: 40, position: [0, 0.4, 11], near: 0.1, far: 100 }}
          gl={{ alpha: true, antialias: true }}
          style={{ background: "#d7d2c6" }}
        >
          <BookScene turned={turned} onTurn={setTurned} />
        </Canvas>
      </div>
      {/* Page controls, sitting directly under the book. Disabled at each end rather
          than hidden, so the control row never shifts position as you leaf through. */}
      <div
        style={{
          display: "flex",
